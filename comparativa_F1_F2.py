...
for i, (sarC, errC) in enumerate(zip([SAR_F1, SAR_F2],
                    [err_SAR_F1, err_SAR_F2])):
...
plt.savefig('comparativa_SAR_F1_F2.png', dpi=300)
plt.show()
...
for i, (hc, err_hc) in enumerate(zip([hc_F1, hc_F2],
                    [err_hc_F1, err_hc_F2])):
...
plt.savefig('comparativa_Hc_F1_F2.png', dpi=300)
plt.show()
...
SAR_arrays_F1, SAR_arrays_F2 = [], []
Hc_arrays_F1, Hc_arrays_F2 = [], []

# Extraer todos los arrays de SAR para cada categoría
for C in res_F1:
    meta, _, _, _, _, Hc, _, _, _, _, _, _, SAR, tau, _ = lector_resultados(C)
    SAR_arrays_F1.append(SAR)
    Hc_arrays_F1.append(Hc)
for C in res_F2:
    meta, _, _, _, _, Hc, _, _, _, _, _, _, SAR, tau, _ = lector_resultados(C)
    SAR_arrays_F2.append(SAR)
    Hc_arrays_F2.append(Hc)
...
graficar_sar_vs_indice(axes[0], SAR_arrays_F1, 'F1', 'tab:blue', 0)
graficar_sar_vs_indice(axes[1], SAR_arrays_F2, 'F2', 'tab:red', 1)
...
# Graficar F1
for i, hc_array in enumerate(Hc_arrays_F1):
    indices = np.arange(len(hc_array))
    ax.plot(indices, hc_array, 's-', alpha=0.7, linewidth=1, markersize=4,
            label=f'F1 - Muestra {i+1}', color='tab:blue')

# Graficar F2
for i, hc_array in enumerate(Hc_arrays_F2):
    indices = np.arange(len(hc_array))
    ax.plot(indices, hc_array, 'o-', alpha=0.7, linewidth=1, markersize=4,
            label=f'F2 - Muestra {i+1}', color='tab:red')
...
plt.savefig('comparativa_Hc_F1_F2_combinado.png', dpi=300)
plt.show()
#%% Comparo los ciclos promedio F1
_,_,_,H_F1_Am,M_F1_Am,_ = lector_ciclos(ciclos_F1[0])
_,_,_,H_F2_Am,M_F2_Am,_ = lector_ciclos(ciclos_F2[0])
...
a.set_title('7A vs F1 (cristal grande)')
a.plot(H_7A_Am/1000,M_7A_Am,label='7A')
a.plot(H_F1_Am/1000,M_F1_Am,label='F1')

b.set_title('3Z vs F2 (cristal chico)')
b.plot(H_3Z_Am/1000,M_3Z_Am,label='3Z')
b.plot(H_F2_Am/1000,M_F2_Am,label='F2')
...
plt.savefig('comparativa_ciclos_promedio_F1_F2.png', dpi=300)
plt.show()
...
a.plot(H_F1_Am/1000,M_F1_Am/max(M_F1_Am),label='F1')

b.set_title('3Z vs F2 (cristal chico)')
b.plot(H_3Z_Am/1000,M_3Z_Am/max(M_3Z_Am),label='3Z')
b.plot(H_F2_Am/1000,M_F2_Am/max(M_F2_Am),label='F2')
...
plt.savefig('comparativa_ciclos_promedio_F1_F2_normalizados.png', dpi=300)
plt.show()